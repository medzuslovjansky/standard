...
This section specifies the inventory and usage of conjunctions, particles, clitics, and other uninflected words in Interslavic. All normative rules use the terms MUST, SHALL, SHOULD, and MAY as defined in RFC 2119.
...
:::caution[Unverified Content]
The source material lists :isv[i], :isv[a], and :isv[ale] but does not elaborate on their semantic differences. The nuance description below is not sourced.
:::

The conjunctions :isv[i] and :isv[a] differ in nuance: :isv[i] introduces a simple additive relation, while :isv[a] introduces a contrastive or adversative relation ("and yet", "but"). The conjunction :isv[ale] is more exclusively adversative.
...
| :isv[li] | whether, if |
| :isv[odkogda / odkogdy] | ever since, since |
| :isv[odkųd] | since |
...
- :isv[Ona prinosi jabloka, da by děti byli zdrave.] — "She brings apples so that the children will be healthy."
...
**Correlative conjunctions** occur in fixed pairs and frame the items they correlate. Both members of the pair MUST be present in the clause.
...
| :isv[koliko … toliko …] | the more … the more … |
...
The particle :isv[li] is a sentential clitic used to form **yes/no questions**. It MUST be placed immediately after the word that carries the focus of the question, typically the finite verb:
...
The particle :isv[či] introduces a **yes/no question** by fronting the entire clause. When used to introduce a question, :isv[či] appears at the very beginning of the sentence:
...
A yes/no question MAY alternatively be formed by intonation alone, without any question particle:

- :isv[Otec kupil knigu?] — "Father bought a book?"

All three constructions (intonation, :isv[či], :isv[li]) are grammatically equivalent for direct yes/no questions.
...
Omission of :isv[ne] before the verb when a negative word is present constitutes a grammatical error in Interslavic.
...
- :isv[Nehaj ona pride!] — "Let her come!"
- :isv[Nehaj živut narodi!] — "Let the peoples live!"

The particle :isv[nehaj] is invariable and does not inflect for person, number, or gender.

:::info[TODO: Committee Input Needed]
The inventory of additional particles (e.g., :isv[da], :isv[že] used as optative/hortative markers, the affirmative particle :isv[da] "yes", the discourse particle :isv[ot], :isv[nu], :isv[to] etc.) requires committee decision on normative status and recommended usage. A supplementary list of common uninflected discourse markers should be specified here.
:::
...
| Person / Case | Dative clitic | Accusative / reflexive clitic |
|---|---|---|
| 1sg | :isv[mi] | :isv[mę] |
| 2sg | :isv[ti] | :isv[tę] |
| 3sg masc./neut. | :isv[mu] | :isv[go] |
| 3sg fem. | :isv[jej] | :isv[ju] |
| 1pl | :isv[nam] | :isv[nas] |
| 2pl | :isv[vam] | :isv[vas] |
| 3pl | :isv[jim] | :isv[jih] |
| Reflexive | — | :isv[sę] |

**Verbal clitics (short forms of :isv[byti]):**

| Form | Function |
|---|---|
| :isv[jesm] | 1sg present copula |
| :isv[jesi] | 2sg present copula |
| :isv[je / jest] | 3sg present copula |
| :isv[jesmo] | 1pl present copula |
| :isv[jeste] | 2pl present copula |
...
2. Dative pronoun clitic (:isv[mi], :isv[ti], :isv[mu], :isv[jej], :isv[nam], :isv[vam], :isv[jim])
3. Accusative pronoun clitic or reflexive :isv[sę] (:isv[mę], :isv[tę], :isv[go], :isv[ju], :isv[nas], :isv[vas], :isv[jih], :isv[sę])

**Example:**

- :isv[On by mu sę znal.] — illustrating conditional > dative > reflexive ordering.
...
- :isv[On je sę myl.] — "He has washed himself." (compound past; :isv[sę] follows auxiliary :isv[je])

Placement of :isv[sę] directly after the participle (e.g., :isv[*On je myl sę]) is non-standard.
