...
### The Weighted Voting System: Formalized Procedure
...
#### Language Weights and the Six-Group Panel
...
#### Step 1: Root Inventory Across Source Languages
...
#### Step 2: Semantic Family Analysis
...
#### Step 3: Form Selection (Majority or Compromise)
...
4. If no single Proto-Slavic reconstruction resolves the tie, both forms MAY be listed as **synonyms** (see the relevant section).

Words are **never borrowed directly** from any single Slavic language. All adopted forms are derived from the Proto-Slavic (or Common Slavic) ancestor by application of the derivation model specified in section This ensures internal phonological consistency and predictable recognition across the entire Slavic spectrum.

#### Native Derivation vs. Pan-Slavic Borrowing
In the majority of cases, the regular application of Interslavic sound correspondences to a Proto-Slavic reconstruction produces a form that is recognizable across modern Slavic languages. However, there are cases where the etymologically regular derivation yields a form that does not exist in any living Slavic language, because all modern languages have either innovated or borrowed a different form.

In such cases, the principle of **maximal recognizability** may override strict etymological regularity. If the etymologically derived form would be opaque to speakers of all modern Slavic languages, while a broadly shared modern form exists (even if it originated as a borrowing in those languages), the modern pan-Slavic form SHOULD be preferred.

:::warning[Unresolved: Etymological Derivation vs. Borrowing of Pan-Slavic Forms]
A concrete example illustrating this tension is the word for "borscht":

- The Proto-Slavic reconstruction \*bъrščь would yield ISV \*bršč (with syllabic r and strong yer) in regular etymological derivation. The etymological register form :isv[bȯršč] (with the strong back yer :latn[ȯ]) implies this derivation.
- However, this form does not exist in any living South Slavic language. The word entered most Slavic languages as a borrowing from East Slavic (specifically Russian :cyrl[борщ]), not through regular native sound change.
- Writing :isv[bȯršč] with a strong yer is etymologically misleading: it implies the word underwent regular yer vocalization in Interslavic (PS \*bъ → ISV :latn[ȯ]), when in reality the :latn[o] in modern forms reflects the East Slavic full-vowel reflex, not yer vocalization.
- The pragmatic solution is to treat :isv[boršč] as a pan-Slavic borrowing (like :isv[vodka] or :isv[vampir]) rather than attempting to derive it from Proto-Slavic through regular sound correspondences.

The general principle: **when native etymological derivation yields forms that do not exist in any living language, it may be better to borrow the modern pan-Slavic form directly**. This applies especially to culturally specific vocabulary (foods, customs, institutions) that spread through contact rather than inheritance.

A systematic review is needed to identify other lexical items where the etymological derivation produces unattested forms and where a pan-Slavic borrowing would be more appropriate.
:::

#### Step 4: Meaning Assignment (Most Average Meaning)
...
When the meaning has drifted significantly in at least one language — especially when the drift creates a potential false cognate — a warning MUST be added to the dictionary entry (see the relevant section).

---

### Frequency-Based Selection Rationale
The selection procedure in the relevant section applies across the entire lexicon, but **priority** is given to high-frequency concepts. The Interslavic lexicon is organized into three frequency tiers:
...
Core-tier entries MUST be fully resolved using the voting procedure before any extended-tier entry for the same semantic field is accepted. Extended-tier entries SHOULD follow the same procedure. Specialized-tier entries MAY rely on international vocabulary  or approved calques  when no broadly understood Slavic root exists.
...
3. Neither form is a high-risk false cognate of the other .
...
Where an internationally recognized Slavic word exists — for example :isv[vampir] "vampire" or :isv[vodka] "vodka" — it is treated as international vocabulary already and requires no further justification for adoption.

---

### Phonological Adaptation Rules for Borrowings
...
|---|---|---|
| Geminated consonant | Simplified to single consonant | *gramma* → :isv[gram] |
| Greek `θ` (th) | Written `t` | *theatro* → :isv[teatr] |
| Greek `φ` (ph) | Written `f` | *phenomenon* → :isv[fenomen] |
| Greek `υ` (y) | Written `i` | *systema* → :isv[sistem] |
| /k/ sound | Always written `k`, never `c` | *contact* → :isv[kontakt] |
| /x/ sound (ch) | Written `h` | *psychology* → :isv[psiholog] |
| /y/ sound (ü, u) | Written `ju` | *bureau* → :isv[bjuro] |
| Intervocalic /z/ (-s-) | Written `z` | *base* → :isv[baza] |
| Intervocalic /s/ (-ss-) | Written `s` | *mass* → :isv[masa] |

---

### Latin and Greek Suffix Adaptation Rules (Merunka)
The seven rules below specify how the most frequent Latin and Greek suffixes MUST be rendered in Interslavic. They supplement the general phonological table in the relevant section and apply in the order listed; the first matching rule wins.

**Rule 1 — Nominal endings -ια / -ia and -η / -ις**

| Source | Interslavic ending | Examples |
|---|---|---|
| Greek `-ια` or Latin `-ia` | `-ija` | :isv[filozofija], :isv[astronomija] |
| Greek `-η` or `-ις` | `-a` | :isv[baza] |

**Rule 2 — Adjectival ending -κος / -cus**

Greek adjectives ending in `-κος` and Latin adjectives ending in `-cus` MUST receive the palatalized Interslavic suffix `-sky`. When the stem-final consonant is `k`, it undergoes the regular Interslavic alternation `k → č` before this suffix.

| Source | Interslavic ending | Examples |
|---|---|---|
| Greek `-κος` / Latin `-cus` | `-sky` | :isv[filosofijsky]; :isv[ekonomičsky] (`k → č`) |

**Rule 3 — Verbal-noun endings -sio, -ssio, -tio / -tia**

| Source | Interslavic ending | Examples |
|---|---|---|
| Latin `-sio` | `-zija` | :isv[televizija] |
| Latin `-ssio` | `-sija` | :isv[diskusija] |
| Latin `-tio` / `-tia` | `-cija` | :isv[gravitacija] |

**Rule 4 — Intervocalic -s- and -ss-; suffix -σμος / -smus**

| Source | Interslavic rendering | Examples |
|---|---|---|
| Latin or Greek `-s-` between vowels | `-z-` | :isv[vaza], :isv[baza], :isv[fizika] |
| Latin or Greek `-ss-` | `-s-` | :isv[masiv] |
| Greek `-σμος` or Latin `-smus` | `-zm` | :isv[kapitalizm] |

**Rule 5 — Nominal endings -o(ν) / -um and -ος / -us**

| Source | Interslavic ending | Examples |
|---|---|---|
| Greek `-ο(ν)` or Latin `-um` | `-um` | :isv[forum] |
| Greek `-ος` or Latin `-us` | `-us` or `-os` | :isv[korpus]; :isv[kosmos] |

Note the following lexicalized exceptions: *museum* → :isv[muzej] (*m. inanim.*, Gen. :isv[muzeja]); *theatrum* → :isv[theatro] (*n.*, Gen. :isv[theatra]).

**Rule 6 — Adjectival ending -στικος / -sticus**

Greek adjectives ending in `-στικος` and Latin adjectives ending in `-sticus` MUST receive the Interslavic suffix `-stičsky`.

| Source | Interslavic ending | Example |
|---|---|---|
| Greek `-στικος` / Latin `-sticus` | `-stičsky` | :isv[kapitalističsky] |

**Rule 7 — Agentive / participial ending -stus / -στος**

Latin or Greek `-stus` / `-στος` MUST be reduced to `-st`.

| Source | Interslavic ending | Example |
|---|---|---|
| Latin `-stus` / Greek `-στος` | `-st` | :isv[kapitalist] |

---

### Morphological Adaptation: Endings for Borrowings
In addition to the phonological rules in the relevant sectiona, international words MUST receive Interslavic inflectional endings in accordance with the following table. The table maps the most common source-language endings (English forms are given as the primary reference) to their Interslavic counterparts.

**Verbs:**

| Source ending | Interslavic ending | Example |
|---|---|---|
| `-ate`, `-fy`, `-ise`/`-ize`, Ger. `-ieren` | `-ovati` | :isv[organizovati] |

**Nouns:**

| Source ending | Interslavic ending | Example |
|---|---|---|
| `-ia`, `-y` (Lat. `-ia`) | `-ija` | :isv[ekonomija] |
| `-sis` (Gr.) | `-za` | :isv[kriza] |
| `-ium` | `-ij` | :isv[helij] |
| `-um`, `-us` | retained | :isv[forum]; :isv[korpus] |
| `-ty` (Lat. `-tas`) | `-tet` | :isv[universitet] |
| `-ics` | `-ika` | :isv[ekonomika] |
| `-ism` | `-izm` | :isv[komunizm] |
| `-ist` | `-ist` | :isv[komunist] |
| `-sion` | `-zija` | :isv[televizija] |
| `-ssion` | `-sija` | :isv[diskusija] |
| `-tion` | `-cija` | :isv[akcija] |

**Adjectives:**

| Source ending | Interslavic ending | Example |
|---|---|---|
| Derived from `-tion`/`-sion` nouns | `-ijny` | :isv[televizijny] |
| `-al` (Lat. `-alis`) | `-alny` | :isv[neutralny] |
| `-ic`, `-ical` (Lat. `-icus`) | `-ičny` | :isv[specifičny] |
| `-ive` (Lat. `-ivus`) | `-ivny` | :isv[pozitivny] |
| `-ous` (Lat. `-osus`) | `-ozny` | :isv[seriozny] |

---

### Borrowings from Non-Latin/Greek Languages (Merunka)
...
|---|---|---|---|
| *metro* | :isv[metro] → :isv[metr-a] | *n.* | French |
| *laser* | :isv[laser] → :isv[laser-a] | *m.* | English |
| *bypass* | :isv[bypass] → :isv[bypass-a] | *m.* | English |

Note: When producing text in Cyrillic, or in any mixed-script context, these loanwords SHOULD be rendered phonetically in Cyrillic in accordance with the general phonetic spelling policy in the relevant section

---

### English Loanwords and Phonetic Spelling
...
| knockout | :isv[nokaut] |
| jazz | :isv[džaz] |
| teenager | :isv[tinejdžer] |
| budget | :isv[budžet] |
| business | :isv[biznes] |
| musical | :isv[mjuzikl] |
| football | :isv[futbol] |
| cocktail | :isv[koktejl] |
...
|---|---|---|---|
| Base | — | :isv[slon] | elephant |
| Simple diminutive | `-ek` | :isv[slonek] | small/young elephant |
| Double diminutive | `-iček` | :isv[sloničеk] | very small elephant; affectionate |
...
|---|---|---|---|
| Base | — | :isv[krava] | cow |
| Simple diminutive | `-ka` | :isv[kravka] | small/young cow |
| Double diminutive | `-ička` | :isv[kravička] | very small cow; affectionate |
...
|---|---|---|---|
| Base | — | :isv[děte] | child |
| Simple diminutive | `-ko` | :isv[děko] | small/young child |
| Double diminutive | `-ičko` | :isv[dětičko] | very small child; affectionate |

Note: for neuter stems belonging to the `-T` declension (e.g., :isv[tele] "calf"), the oblique stem in `-et-` is used before the diminutive suffix: :isv[tele] → :isv[teletko] (Gen. :isv[teletka]). See the relevant section (declension) for the `-T` paradigm.

Note: some masculine nouns form their simple diminutive with the neuter-type suffix `-ko`, in which case they retain masculine gender but follow the :isv[sělo] declension in the singular: e.g., :isv[brat] "brother" → :isv[bratko] (*m.*, Gen. sg. :isv[bratka], declined as :isv[sělo]).

Cross-reference: the derivational affixes `-ica` (feminine agent/diminutive) and `-ec` (masculine agent/diminutive, with loss of thematic `-e-`) are covered in the morphology section  and are distinct from the productive diminutive paradigm described here.

---

## False Cognate Register
### Definition and Classification
A **false cognate** (also: false friend; :isv[lažny drug]) is a word whose phonological or orthographic form is similar — or identical — across two or more Slavic languages but whose meaning diverges between them, such that a reader familiar with form A in language X may incorrectly assume meaning B when encountering the same form in Interslavic or language Y.
...
Only high-risk and medium-risk false cognates are subject to the mandatory dictionary warning in the relevant section

---

### High-Risk False Cognate Pairs
...
|---|---|---|---|
| :isv[duma] | "thought, reflection" | "pride" | Polish |
| :isv[listopad] | "falling of leaves; October" | "November" | Polish, Czech |
| :isv[uroda] | "harvest; produce" | "beauty" | Polish; "ugly person, freak" | Russian, Czech |
| :isv[zbožje] | "grain, cereal" | "goods, merchandise" | some Slavic varieties |
| :isv[pozor] | "attention, care" | "disgrace, shame" | Russian, Bulgarian |
| :isv[zavisnost] | "dependence" | "envy" | some South Slavic varieties |
...
### Avoidance and Disambiguation Policy
...
Slight divergences in meaning — where the Interslavic sense and the divergent sense belong to the same semantic area and misunderstanding is unlikely in context — do not require a warning. For example, the fact that :isv[ljubiti] means "to love" in most Slavic languages but "to like" in Polish is a low-risk divergence that does not warrant a mandatory warning.

---

## Calque vs. Borrowing Policy
### Preference Ordering
...
3. **Calque (loan translation)**: A morpheme-by-morpheme translation of an international term into Interslavic roots, following the formation rules in the relevant section
4. **Adapted international borrowing**: An international (Greco-Latin or widely recognized) term adapted according to –
5. **Phonetic borrowing**: A word borrowed from a non-Slavic language (including English) adapted phonetically per , used only when no option above is feasible.
...
### Calque Formation Rules
Calques MUST be formed in accordance with the following rules:

1. Every morpheme in the calque MUST be either an attested Interslavic root or a productive Interslavic affix (see the relevant section).
...
|---|---|---|
| skyscraper | :isv[nebodrap] | :isv[nebo] "sky" + :isv[drapati] "to scratch" |
| earthquake | :isv[zemjetrěs] | :isv[zemlja] "earth" + :isv[trěsti] "to shake" |
| wheelchair | :isv[kolokrěslo] | :isv[kolo] "wheel" + :isv[krěslo] "armchair" |
...
## Neologism Approval Criteria
### Formal Requirements for New Words
A proposed neologism MUST satisfy all of the following formal conditions before it can be submitted for review:

1. The proposed form MUST conform to Interslavic **phonotactic rules** as specified in the relevant section
2. The proposed form MUST use only **attested Slavic roots** (traceable to Proto-Slavic), **approved international vocabulary** , or **productive Interslavic affixes** .
...
- **Regional differentiation**: The proposed word is more broadly understood in a geographic subgroup of the Slavic speech area than the existing word, and the existing word functions as the synonym for the complementary subgroup (see the relevant section).

In the case of portmanteau words — forms that blend phonological material from multiple Slavic cognates to maximize pan-Slavic recognizability — the proposer MUST demonstrate that the blend is more readily understood by Slavic speakers than either of the contributing words alone. Examples include :isv[katka] "duck" (from Russian *utka*, Polish *kaczka*, South Slavic *patka*).
...
## Internationalism Adaptation Checklist
The following checklist provides a step-by-step procedure for adapting any international word to Interslavic orthography and morphology. Cross-references to –a are provided at each relevant step.

| Step | Action | Rule reference |
|---|---|---|
| 1 | Verify that the word is recognized in at least 4 of the 6 subgroups. |  |
| 2 | Apply general phonological adaptations: simplify geminates, adapt θ/φ/υ/k/ch/ü/intervocalic-s. |  |
| 2a | If the word is of Latin or Greek origin, apply the seven suffix-specific adaptation rules in the listed order (Rules 1–7). | a |
| 3 | Assign the correct morphological ending based on the source-language suffix. |  |
| 3a | If the word is not of Latin or Greek origin but uses Latin script, retain the original spelling and assign Interslavic grammatical categories (gender, declension). | a |
| 4 | If the word is of English origin and will appear in Cyrillic, apply phonetic spelling. |  |
| 5 | Check the result against the false cognate register. |  |
| 6 | Confirm that the adapted form is formally distinct from all existing Interslavic entries. |  |
| 7 | Record the etymology, source form, and adaptation decisions in the dictionary entry. | — |
| 8 | If the word fails step 1, proceed to the calque/borrowing preference order. |  |
