...
Note: When producing text in Cyrillic, or in any mixed-script context, these loanwords SHOULD be rendered phonetically in Cyrillic in accordance with the general phonetic spelling policy in §8.2.4.
...
Note: for neuter stems belonging to the `-T` declension (e.g., :isv[tele] "calf"), the oblique stem in `-et-` is used before the diminutive suffix: :isv[tele] → :isv[teletko] (Gen. :isv[teletka]). See §4 (declension) for the `-T` paradigm.
