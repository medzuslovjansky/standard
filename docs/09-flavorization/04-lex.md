...
The Interslavic dictionary marks many synonyms with a regional preference label indicating which part of the Slavic territory the word is primarily associated with.
The labels used are:

| Label | Meaning |
|---|---|
| **North** | Primarily attested in North Slavic (Russian, Belarusian, Polish, Sorbian) |
| **South** | Primarily attested in South Slavic (Serbian, Croatian, Bosnian, Slovene, Bulgarian, Macedonian) |
| **East** | Primarily attested in East Slavic (Russian, Ukrainian, Belarusian) |
| **West** | Primarily attested in West Slavic (Polish, Czech, Slovak, Sorbian) |

A synonym marked **North** is not incorrect in Southern contexts; it is simply less likely to be recognized immediately by a South Slavic audience.
The principle governing lexical flavorization is therefore: **for a given target audience, prefer the synonym whose regional distribution best overlaps with that audience.**

:::info[TODO: Committee Input Needed]
The committee should specify the formal criteria by which a synonym pair receives a North/South/East/West label in the dictionary, and whether any words can bear multiple labels simultaneously (e.g., East+South). The relationship between lexical flavorization labels and the phonological voting weight system also requires clarification.
:::
...
| this | :isv[tot] / :isv[ta] / :isv[to] | :isv[taj] / :isv[ta] / :isv[to] | |
| that | :isv[onen] / :isv[ona] / :isv[ono] | :isv[onaj] / :isv[ona] / :isv[ono] | |
...
:::caution[Unverified Content]
The `potomu čto`/`jer`, `čtoby`/`da bi`, and `hotja`/`mada`/`premda` alternations are not discussed in the source material. Only the `dlja`/`za` alternation is sourced.
:::

| because | :isv[potomu čto] / :isv[ibo] | :isv[zato što] / :isv[jer] |
| in order to | :isv[čtoby] | :isv[da bi] |
| although | :isv[hotja] | :isv[mada] / :isv[premda] |

Regional reference for 'for (purposive)':
...
| Meaning | Northern | Southern |
|---|---|---|
| to study, learn | :isv[vyučiti] | :isv[izučiti] |
| to speak out, pronounce | :isv[vygovoriti] | :isv[izgovoriti] |
...
The following list covers frequently occurring synonyms where Northern and Southern lexical choices diverge perceptibly.

| Meaning | Northern | Southern |
|---|---|---|
| city | :isv[gorod] | :isv[grad] |
| time (duration) | :isv[vremja] | :isv[vrěme] |
| eye | :isv[glaz] | :isv[oko] |
| snow | :isv[sneg] | :isv[sněg] |
| hand | :isv[ruka] | :isv[ruka] |
| word | :isv[slovo] | :isv[reč] / :isv[rěč] |
| always | :isv[vsegda] | :isv[uvěk] / :isv[vazda] |
| already | :isv[uže] | :isv[već] |
| only | :isv[toliko] | :isv[samo] |
| very | :isv[očenь] | :isv[veoma] / :isv[jako] |
| because | :isv[potomu čto] | :isv[jer] |
| now | :isv[teper] | :isv[sada] |

Regional reference for 'city':

| Polish | Ukrainian | Bulgarian | Czech | Serbian |
|---|---|---|---|---|
| `gród` / `miasto` | `місто` | `град` | `město` | `град` |
...
| Meaning | Northern | Southern |
|---|---|---|
| but | :isv[no] / :isv[jednak] | :isv[ali] / :isv[nego] |
| or | :isv[ili] | :isv[ili] / :isv[al'] |
| if | :isv[esli] / :isv[jesli] | :isv[ako] |
| when | :isv[kogda] | :isv[kad] / :isv[kada] |
| until | :isv[poka] | :isv[dok] |
| after | :isv[posle] | :isv[posle] / :isv[nakon] |

Regional reference for 'if':

| Polish | Ukrainian | Bulgarian | Czech | Serbian |
|---|---|---|---|---|
| `jeśli` | `якщо` / `якби` | `ако` | `jestliže` / `pokud` | `ако` |
...
**Standard Interslavic:**

> :isv[Člověk, ktory žije v velikom gorode, ne vsegda imaje čas dlja čitanja knih.]

---

**Northern lexical flavorization:**

> :isv[Člověk, **ktory** žije v velikom **gorode**, ne **vsegda** imaje čas **dlja** čitanja knih.]

Lexical choices (Northern):
- :isv[ktory] [Northern relative pronoun; Southern alternative: :isv[koj]]
- :isv[gorod] [Northern word for city; Southern alternative: :isv[grad]]
- :isv[vsegda] [Northern word for always; Southern alternative: :isv[uvěk]]
- :isv[dlja] [Northern purposive preposition; Southern alternative: :isv[za]]

---

**Southern lexical flavorization:**

> :isv[Člověk, **koj** žije v velikom **gradu**, ne **uvěk** imaje čas **za** čitanje knih.]

Lexical choices (Southern):
- :isv[koj] [Southern relative pronoun; Northern alternative: :isv[ktory]]
- :isv[grad] [Southern word for city; Northern alternative: :isv[gorod]]
- :isv[uvěk] [Southern word for always; Northern alternative: :isv[vsegda]]
- :isv[za] [Southern purposive preposition; Northern alternative: :isv[dlja]]
