...
| Standard Latin | Polish-based Latin | Polish analogy | Notes |
|---|---|---|---|
| :isv[v] | `w` | Polish _w_ | Polish has no `v` |
| :isv[č] | `cz` | Polish _cz_ | digraph for [tʃ] |
| :isv[š] | `sz` | Polish _sz_ | digraph for [ʃ] |
...
| Etymological :isv[ų] | `ą` | Polish _ą_ | from etymological alphabet |

**Complete character substitution table (Latin standard → Polish-based):**

| Standard | Polish-based | Standard | Polish-based |
|---|---|---|---|
| `a` | `a` | `p` | `p` |
| `b` | `b` | `r` | `r` |
| `c` | `c` | `s` | `s` |
...
| Standard Latin | Polish-based Latin |
|---|---|
| :isv[Medžuslovjansky jest orudje dlja komunikacije s Slovjanami.] | _Miedżusłowiański jest orudie dla komunikacije s Słowianami._ |
| :isv[Učenje ne jest težko i ne trovaje dolgo.] | _Uczenie nie jest tiażko i nie trowaje dołgo._ |

Regional reference (characters used in each language):

| Standard form | Polish | Ukrainian | Bulgarian | Czech | Serbian |
|---|---|---|---|---|---|
| `v` | `w` | `в` | `в` | `v` | `v` |
| `č` | `cz` | `ч` | `ч` | `č` | `č` |
| `š` | `sz` | `ш` | `ш` | `š` | `š` |
...
| Standard Latin | Czech/Slovak-based | Notes |
|---|---|---|
| :isv[y] | `y` | retained; Czech/Slovak distinguish y and i |
| :isv[ě] | `ě` | Czech uses `ě`; Slovak uses `ie` — author may choose |
...
| Standard Latin | Czech/Slovak-based Latin |
|---|---|
| :isv[Medžuslovjansky jest orudje dlja komunikacije s Slovjanami.] | _Medžuslovjansky jest orudje dlja komunikacije s Slovjanami._ |

For Czech audiences, the standard Latin alphabet requires minimal change; the primary orthographic adaptation is the optional :isv[g] → `h` substitution and the treatment of :isv[nj] as `ň`.
...
| Standard Latin | Croatian-based Latin |
|---|---|
| :isv[Medžuslovjansky jest orudje dlja komunikacije s Slovjanami.] | _Međuslovjanski jest orudje dlja komunikacije s Slovjanami._ |
| :isv[Učenje ne jest težko i ne trovaje dolgo.] | _Učenje ne jest težko i ne trăvaje dălgo._ |

Regional reference:

| Standard form | Polish | Ukrainian | Bulgarian | Czech | Serbian / Croatian |
|---|---|---|---|---|---|
| `y` | `y` | `и` | `и` | `y` | `i` |
| `šč` | `szcz` | `щ` | `щ` (= `št`) | `šč` | `šт` |
| `ě` | `ie` | `і` | `—` | `e` | `ije` / `e` |
...
| Standard Cyrillic | East Slavic Cyrillic | Notes |
|---|---|---|
| `ј` | `й` (after vowel) or omitted before vowel | East Slavic `й` is [j]; before vowels East Slavic uses vowel letters `я`, `ю`, `е`, `ё` |
| `ја` | `я` | East Slavic convention |
| `ју` | `ю` | East Slavic convention |
| `је` | `е` / `ье` | East Slavic convention |
| `јо` | `ё` (in Russian contexts) | East Slavic convention |
| `љ` | `ль` | soft sign after `л` |
| `њ` | `нь` | soft sign after `н` |
| `ы` | `ы` | retained; East Slavic standard |
| `є` | `е` | Ukrainian `є` → Russian/Belarusian `е` |
| `ѣ` (`ě`) | `е` | East Slavic reflex of ě |

**Complete character substitution table (Standard Cyrillic → East Slavic Cyrillic):**

| Standard | East Slavic | Standard | East Slavic |
|---|---|---|---|
| `а` | `а` | `р` | `р` |
| `б` | `б` | `с` | `с` |
| `в` | `в` | `т` | `т` |
...
| Standard Cyrillic | East Slavic Cyrillic |
|---|---|
| `Меджусловјанскы јест орудје дља комуникације с Словјанами.` | `Меджусловяньски ест орудье для комуникацие с Словянами.` |
| `Ученје не јест тежко и не троваје долго.` | `Ученье не ест тяжко и не тровае долго.` |

Regional reference:

| Standard form | Russian | Ukrainian | Bulgarian | Macedonian | Serbian |
|---|---|---|---|---|---|
| `ј` | `й` / vowel letters | `й` / vowel letters | `й` / vowel letters | `ј` | `ј` |
| `љ` | `ль` | `ль` | `л` | `љ` | `љ` |
| `њ` | `нь` | `нь` | `н` | `њ` | `њ` |
| `є` | `е` | `є` | `е` | `е` | `е` |
...
| Standard Cyrillic | Serbian/Macedonian Cyrillic |
|---|---|
| `Меджусловјанскы јест орудје дља комуникације с Словјанами.` | `Меджусловјански јест орудје дља комуникације с Словјанами.` |

The primary change in this example is `скы` → `ски` (y → i substitution).
...
> :isv[Medžuslovjansky jest orudje dlja komunikacije s Slovjanami.]

---

| Orthographic flavor | Text |
|---|---|
| **Standard Latin** | :isv[Medžuslovjansky jest orudje dlja komunikacije s Slovjanami.] |
| **Polish-based Latin** | _Miedżusłowiański jest orudie dla komunikacije s Słowianami._ |
| **Czech/Slovak-based Latin** | _Medžuslovjansky jest orudje dlja komunikacije s Slovjanami._ |
| **Croatian/Slovene-based Latin** | _Međuslovjanski jest orudje dlja komunikacije s Slovjanami._ |
| **Standard Cyrillic** | _Меджусловјанскы јест орудје дља комуникације с Словјанами._ |
| **East Slavic Cyrillic** | _Меджусловяньски ест орудье для комуникацие с Словянами._ |
| **Serbian/Macedonian Cyrillic** | _Меджусловјански јест орудје дља комуникације с Словјанами._ |

---

**Second example sentence:**

> :isv[Učenje ne jest težko i ne trovaje dolgo.]

| Orthographic flavor | Text |
|---|---|
| **Standard Latin** | :isv[Učenje ne jest težko i ne trovaje dolgo.] |
| **Polish-based Latin** | _Uczenie nie jest tiażko i nie trowaje dołgo._ |
| **Czech/Slovak-based Latin** | _Učeňe ne jest težko i ne trovaje dolho._ |
| **Croatian/Slovene-based Latin** | _Učenje ne jest težko i ne trovaje dolgo._ |
| **Standard Cyrillic** | _Ученје не јест тежко и не троваје долго._ |
| **East Slavic Cyrillic** | _Ученье не ест тяжко и не тровае долго._ |
